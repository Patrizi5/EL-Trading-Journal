...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-shell-dark text-shell-text font-shell p-6">
      {/* Header */}
      ...
      <div className="bg-shell-card rounded-xl p-4 mb-6 border border-shell-accent shadow-xl shadow-black/30">
      ...
      <div className="bg-shell-card rounded-xl p-4 mb-6 border border-shell-accent shadow-xl shadow-black/30">
        ...
        <div className="flex gap-3 mb-4 items-center flex-wrap">
          ...
          >
            {Object.entries(MARKETS).map(([k, v]) => <option key={k} value={k}>{v.name}</option>)}
          </select>
          ...
          <select 
            ...
            className="bg-shell-accent text-shell-text px-3 py-2 rounded border border-shell-glow focus:outline-none focus:border-shell-green"
          >
            <option value="long" className="text-black">Long</option>
        ...
        {symbol && <LiveChart symbol={symbol} />}
      </div>
      ...
      <div className="bg-shell-card rounded-xl p-4 mb-6 border border-shell-accent shadow-xl shadow-black/30">
        ...
        <ul className="space-y-3">
          {trades.map(t => (
            <li key={t.id} className="bg-shell-accent rounded-lg p-4 flex items-center justify-between border border-shell-glow hover:border-shell-green transition-all">
              <div>
                <div className="font-semibold">{t.symbol} • {t.side.toUpperCase()}</div>
                <div className="text-sm text-shell-text/70">Entry: {t.entry}</div>
                {t.exit && <div className="text-sm text-shell-text/70">Exit: {t.exit} • P&L: ${t.pnl?.toFixed(2)}</div>}
              ...
              <div className="flex items-center gap-2">
                ...
                {t.psych?.pre && (
                  <span className={`px-2 py-1 rounded text-xs font-bold ${
                    calculateERS(t.psych) > 60 ? 'bg-shell-red text-white' : 
                    calculateERS(t.psych) > 30 ? 'bg-yellow-600 text-black' : 
