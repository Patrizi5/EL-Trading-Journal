...
const STRATEGIES = ['breakout', 'momentum', 'reversal', 'scalp', 'swing', 'earnings', 'news', 'technical', 'support_resistance', 'trend_following', 'mean_reversion', 'other'];
const EMOTIONS = {
...
export default function TradeForm({ open, onClose, onSave, trade, isLoading }) {
  ...
  const handleSubmit = () => {
    ...
    const data = {
      ...
      profit_loss: pl,
      profit_loss_percent: pl && entry ? (pl / (entry * parseFloat(formData.quantity))) * 100 : undefined,
      result_r: resultR
    };
      ...
      <DialogContent className="bg-slate-900 border-slate-800 text-white max-w-3xl max-h-[90vh] overflow-y-auto">
        ...
        <Tabs defaultValue="details" className="w-full">
          <TabsList className="bg-slate-800 w-full grid grid-cols-4">
            ...
            <TabsTrigger value="risk" className="data-[state=active]:bg-slate-700">Risk</TabsTrigger>
            <TabsTrigger value="notes" className="data-[state=active]:bg-slate-700">Notes</TabsTrigger>
            <TabsTrigger value="psychology" className="data-[state=active]:bg-slate-700">Psychology</TabsTrigger>
          </TabsList>
          ...
          <TabsContent value="details" className="space-y-4 mt-4">
            ...
            <div className="grid grid-cols-3 gap-4">
              ...
              <div>
                ...
                <Select value={formData.strategy} onValueChange={(v) => handleChange('strategy', v)}>
                  <SelectTrigger className="bg-slate-800 border-slate-700 mt-1">
            ...
            {formData.status === 'closed' && (
              <>
                ...
                {calculatePL() !== null && (
                  <div className={`p-4 rounded-xl ${calculatePL() >= 0 ? 'bg-emerald-500/10 border border-emerald-500/20' : 'bg-rose-500/10 border border-rose-500/20'}`}>
                    <div className="flex items-center justify-between">
                      <div>
                        <p className="text-slate-400 text-sm">Result</p>
                        <p className={`text-2xl font-bold ${calculatePL() >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}>
                          {calculatePL() >= 0 ? '+' : ''}${calculatePL().toFixed(2)}
                          {calculatePL() && formData.entry_price && (
                            <span className="text-lg ml-2">
                              ({(calculatePL() / (parseFloat(formData.entry_price) * parseFloat(formData.quantity)) * 100).toFixed(2)}%)
                            </span>
                          )}
                        </p>
                      </div>
                      {calculateResultR() !== null && (
                        <div className="text-right">
                          ...
                          <p className={`text-2xl font-bold ${calculateResultR() >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}>
                            {calculateResultR() >= 0 ? '+' : ''}{calculateResultR().toFixed(2)}R
                          </p>
                        </div>
                      )}
                    </div>
              ...
              <div className="p-4 rounded-xl bg-blue-500/10 border border-blue-500/20">
                ...
                <div className="grid grid-cols-3 gap-4 text-sm">
                  ...
                  <div>
                    ...
                    <p className="text-blue-400 font-semibold">
                      ${(Math.abs(parseFloat(formData.entry_price) - parseFloat(formData.stop_loss)) * parseFloat(formData.quantity)).toFixed(2)}
                    </p>
                  </div>
                  <div>
                    ...
                    <p className="text-blue-400 font-semibold">
                      {formData.take_profit ? 
                        `1:${(Math.abs(parseFloat(formData.take_profit) - parseFloat(formData.entry_price)) / Math.abs(parseFloat(formData.entry_price) - parseFloat(formData.stop_loss))).toFixed(2)}` 
                        : 'N/A'}
                    </p>
                  </div>
                </div>
                ...
                <div className="mt-1">
                  {formData.screenshot_url ? (
                    <div className="relative">
                      <img src={formData.screenshot_url} alt="Entry Chart" className="rounded-lg max-h-40 object-cover w-full" />
                      <Button 
                        size="icon" 
                        variant="destructive" 
              ...
              <div>
                ...
                <div className="mt-1">
                  {formData.chart_image_url ? (
                    <div className="relative">
                      <img src={formData.chart_image_url} alt="Exit Chart" className="rounded-lg max-h-40 object-cover w-full" />
                      <Button 
                        size="icon" 
                        variant="destructive" 
                        className="absolute top-2 right-2 h-7 w-7"
          ...
          <TabsContent value="psychology" className="space-y-4 mt-4">
            <div>
              ...
              <p className="text-slate-500 text-xs mb-2">Deep dive into your mental state and decision-making</p>
              <Textarea
              ...
              <div className="flex flex-wrap gap-2">
                {EMOTIONS.entry.map(emotion => (
                  <Button
                    ...
                    className={`capitalize ${formData.emotion_entry === emotion ? 'bg-emerald-600 hover:bg-emerald-700' : 'border-slate-700'}`}
              ...
              <div className="flex flex-wrap gap-2">
                {EMOTIONS.exit.map(emotion => (
                  <Button
                    ...
                    className={`capitalize ${formData.emotion_exit === emotion ? 'bg-emerald-600 hover:bg-emerald-700' : 'border-slate-700'}`}
            ...
            <div>
              ...
              <p className="text-slate-500 text-xs mb-2">How well did you follow your trading plan? (1 = Poor, 5 = Perfect)</p>
              <div className="flex gap-2 mt-2">
                {[1, 2, 3, 4, 5].map(rating => (
                  <Button
                    key={rating}
                    variant={formData.discipline_rating === rating ? "default" : "outline"}
                    className={`flex-1 ${formData.discipline_rating === rating ? 'bg-amber-600 hover:bg-amber-700' : 'border-slate-700'}`}
                    onClick={() => handleChange('discipline_rating', rating)}
                  >
                    <Star className={`w-4 h-4 ${formData.discipline_rating >= rating ? 'fill-current' : ''}`} />
                  </Button>
                ))}
              </div>
            </div>
        ...
        <DialogFooter className="mt-6">
          ...
          >
            {isLoading ? <Loader2 className="w-4 h-4 animate-spin" /> : trade ? 'Update Trade' : 'Save Trade'}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
